...
const Home = () => {
  ...
  const [query, setQuery] = useState({ season: currentlyAiringAnime.season, year: currentlyAiringAnime.year });
  ...
  const handleTabChange = () => {
    if(query.season === currentlyAiringAnime.season
       && query.year === currentlyAiringAnime.year){
      setTabText("Upcoming Season");
      setTabValue("airing");
    } else if (query.season === Seasons[(Seasons.findIndex((s) => s === currentlyAiringAnime.season) + 1) % Seasons.length]) {
      setTabText("Upcoming Season");
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100">
      ...
      <header className="border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <Link href='/' className="text-2xl font-bold">AnimeTracker</Link>
          {/* Search */}
          <Button className='bg-zinc-800 hover:bg-zinc-700 dark:bg-zinc-700 dark:hover:bg-zinc-600'>My List</Button>
        </div>
      </header>

      {/* Main Content */}
      <main className="container mx-auto px-4 py-8">
        <Tabs value={tabValue} onValueChange={(value) => setTabValue(value)} className="w-full">
          ...
          <TabsList className="bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 mb-6 w-full max-w-md">
            <TabsTrigger onClick={() => setQuery(currentlyAiringAnime)} value="airing" className="w-1/2 data-[state=active]:bg-white dark:data-[state=active]:bg-zinc-700 data-[state=active]:text-zinc-900 dark:data-[state=active]:text-zinc-100">
              Currently Airing
            </TabsTrigger>
            <TabsTrigger onClick={() => {
              ...
              setQuery({...query, season: Seasons[(Seasons.findIndex((s) => s === currentlyAiringAnime.season) + 1) % Seasons.length]})
            }} value="queryAnimes" className="w-1/2 data-[state=active]:bg-white dark:data-[state=active]:bg-zinc-700 data-[state=active]:text-zinc-900 dark:data-[state=active]:text-zinc-100">
              {tabText}
            </TabsTrigger>
          </TabsList>
      ...
      <footer className="bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-200 dark:border-zinc-800 py-6">
        <div className="container mx-auto px-4 text-center">
          <p className="text-zinc-500 dark:text-zinc-400 text-sm">
            © {new Date().getFullYear()} AnimeTracker. All anime data is for demonstration purposes only.
          </p>
        </div>
      </footer>
    </div>
